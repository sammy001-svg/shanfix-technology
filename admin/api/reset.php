...
try {
    $stmt = $pdo->prepare("SELECT * FROM password_resets WHERE token = ? AND role = 'admin' AND used = 0 AND expires_at > NOW()");
    $stmt->execute([$token]);
    ...
    $pdo->prepare("UPDATE users SET password = ? WHERE email = ? AND role = 'admin'")->execute([$hashed, $reset['email']]);
